...
let isHTMLElementInformation = (x: any): x is HTMLElementInformation => {
    return !isTextElementInformation(x) && typeof (x as HTMLElementInformation).type != "function";
};

let isFunctionComponentInformation = (x: any): x is FunctionComponentInformation => {
...
type ElementInformation = HTMLElementInformation | TextElementInformation | FunctionComponentInformation;
...
function render(element: ElementInformation, container_: HTMLElement) {
    ...
    function wookLoop(deadline: IdleDeadline): void {
        ...
        function createChildrenFiber(
            fiber: FiberInformation & (HTMLElementInformation | FunctionComponentInformation),
            children: ElementInformation[]
        ) {
        ...
        function commitRoot(root: HTMLElementInformation & FiberInformation) {
            function updateDom(dom: HTMLElement | Text, oldProps: any, newProps: any) {
                ...
                if (isHTMLElement(dom)) {
                    const keys = [...Object.keys(oldProps), ...Object.keys(newProps)].filter((key) => key != "children");
                    keys.forEach((key) => {
                        if (newProps[key]) {
                            if (key.startsWith("on")) {
                                if (oldProps[key]) {
            ...
            function commitWork(fiber: Fiber) {
                ...
                if (!fiber.dom) {
                    ...
                    if (fiber.effectTag == "PLACEMENT") {
                        ...
                        while (sibling && (sibling.effectTag == "PLACEMENT" || !sibling.dom)) {
        ...
        // 处理类似App这种函数式组件节点
        function dealingFunctionComponent(nextUnitOfWork: FunctionComponentInformation & FiberInformation) {
        ...
        function dealingHostComponent(nextUnitOfWork: (HTMLElementInformation | TextElementInformation) & FiberInformation) {
            if (nextUnitOfWork.effectTag == "PLACEMENT") nextUnitOfWork.dom = createDom(nextUnitOfWork);
        ...
        if (deadline.timeRemaining() > 30 && nextUnitOfWork != null) {
            if (isHTMLElementInformation(nextUnitOfWork) || isTextElementInformation(nextUnitOfWork)) {
...
function useState(initial: any) {
    ...
    if (nextUnitOfWork.hooks.length - 1 >= hookIndex) {
        ...
        workInProgressFiber.hooks[hookIndex].actions.forEach((fn) => {
            workInProgressFiber.hooks[hookIndex].state = fn(workInProgressFiber.hooks[hookIndex].state);
        });
    ...
    function setState(actionOrValue: ((state: any) => any) | any) {
        tmp++;
        if (tmp == 1) {
            console.log(workInProgressFiber.hooks[index].actions);
        }
        if (tmp == 2) debugger;
